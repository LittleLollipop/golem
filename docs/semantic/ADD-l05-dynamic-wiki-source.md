...
- **两种模式**：
  - `topics`（默认）：沿用原 6 个选题（光合作用/热力学第二定律/线粒体/古腾堡印刷术/板块构造/递归），但每条的**正文与 URL 都实时从 wiki 取**；保留 rank 升序与 "top1 学过则 top2" 叙事、按实例去重完全不变。
...
## 接入点（index.ts 组合根）
- env 可切换：`FAKEREN_KNOWLEDGE_SOURCE=static|wikipedia`（默认 **wikipedia**）、`FAKEREN_KNOWLEDGE_LANG`（默认 zh）、`FAKEREN_KNOWLEDGE_MODE`（默认 topics）。
- 启动打印当前后端：`[fakeren] L0.5 knowledge source = wikipedia(zh/topics)`。
...
- 注意：topics 模式 6 条学完后当日不再有新学（要无尽发现请用 `mode=random`）。

## 业务层测试
`tests/wikipedia-source.test.ts`（mock fetch，不触网）：
- topics：实时取回 + 按 rank 升序 + 真实 URL；
- topics 单条失败跳过、其余保留；全失败返回 `[]`；
- random：单条 rank1 + `wiki-<slug>` id；无法解析返回 `[]`。
全量测试 120 passed（含原本 static 用例保留）。
